...
export const getStoredData = () => {
  ...
  try {
    const p = localStorage.getItem("jira_projects");
  ...
  try {
    const u = localStorage.getItem("jira_people");
    if (u) people = JSON.parse(u);
  } catch (e) {
    localStorage.removeItem("jira_people");
  }

  try {
    const l = localStorage.getItem("jira_logs");
    if (l) logs = JSON.parse(l);
  } catch (e) {
    localStorage.removeItem("jira_logs");
  }

  return { projects, people, logs };
};
