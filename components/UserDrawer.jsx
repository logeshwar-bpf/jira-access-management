...
export default function UserDrawer({ user, projects, onClose, onToggleAccess }) {
  ...
  const filteredProjects = projects.filter(
    (proj) =>
      ...
      proj.key.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      <div className="w-full max-w-2xl h-full bg-[var(--bg)] border-l border-[var(--border)] p-6 flex flex-col shadow-2xl overflow-y-auto">
        ...
        <div className="flex items-center justify-between pb-5 border-b border-[var(--border)]">
          <div className="flex items-center gap-3">
            <div className={`w-11 h-11 rounded-xl ${user.avatarBg || "bg-[#5B5BD6]"} flex items-center justify-center text-white font-extrabold text-base shadow-md`}>
